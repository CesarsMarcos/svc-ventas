...
public class RestExceptionHandler {
  ...
  public ResponseEntity<?> handleDuplicateVenta(DataIntegrityViolationException ex) {
	...
	public ResponseEntity<?> handleAuthorizationDenied(AuthorizationDeniedException ex) {
		return ResponseEntity.status(HttpStatus.FORBIDDEN)
						.body(Collections.singletonMap("mensaje", Constantes.RESPONSE_ERROR_403));
	}
  ...
  private String obtenerMensajePorConstraint(String constraintName) {
    ...
    return switch (constraintName) {
      case "uk_venta_serie_correlativo" -> "Ya existe una venta con ese serie y correlativo";

      case "uk_usuario_username" -> "El username ya está en uso";

      case "uk_producto_codigo" -> "El código del producto ya existe";

      default -> "Violación de restricción: " + constraintName;
    };
  }
